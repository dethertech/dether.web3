...
const sellEthForERC20 = async (wallet, gasPrice, signedOrder) => {
  ...
  const fillTsx = await airswapExchange.functions['fill(address,uint256,address,address,uint256,address,uint256,uint256,uint8,bytes32,bytes32)'](
...
const setAllowanceIfNeeded = async (wallet, sellToken, gasPrice) => {
  const payToken = getTokenContract(wallet, sellToken, gasPrice);
  const allowance = await payToken.allowance(wallet.address, getAirsSwapExchangeContractAddr(wallet.provider));
  ...
  if (allowance.lt(getMaxUint256Value().div(2))) {
    ...
    const sentTsx = await payToken.approve(getAirsSwapExchangeContractAddr(wallet.provider), getMaxUint256Value());
      ...
      switch (state) {
        ...
        case AirswapStates.RcvFindIntentsRes: {
          ...
          if (sellToken !== 'ETH') {
            ...
            setAllowanceIfNeeded(wallet, sellToken, gasPrice).then((tsxCount_) => {
...
export const getRateEstimation = async ({ sellToken, buyToken, sellAmount }) => {
  ...
  return parseFloat(customRateBN);
};
